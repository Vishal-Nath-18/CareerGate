...
function createRelativeImportRewritePlugin(filename: string) {
  const fromDir = filename.substring(0, filename.lastIndexOf("/"));

  return function relativeImportRewritePlugin() {
    return {
      visitor: {
        "ImportDeclaration|ExportNamedDeclaration|ExportAllDeclaration"(path: any) {
          ...
          if (value.startsWith("./") || value.startsWith("../")) {
            const resolved = resolveRelativePath(fromDir, value);
            source.value = resolved.substring(1); // bare specifier — matches the import map's existing keys
          }
        },
      },
    };
...
export function transformJSX(
  ...
  try {
    ...
    const result = Babel.transform(processedCode, {
      ...
      plugins: [createRelativeImportRewritePlugin(filename)],
    });
  ...
  ${errors.length === 0 ? `<script type="module">
    ...
    class ErrorBoundary extends React.Component {
      ...
      componentDidCatch(error, errorInfo) {
        ...
        window.parent.postMessage({ type: 'preview-error', message: error.toString(), stack: error.stack || '' }, '*');
      }

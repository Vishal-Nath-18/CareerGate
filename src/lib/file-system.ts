import { findUndefinedSetters } from "./validate-react-code";
...
export class VirtualFileSystem {
  ...
  private validateContent(path: string, content: string): string {
    if (typeof window !== "undefined") return ""; // client mirror — never validate here, only the server's tool result matters
    if (!/\.(jsx?|tsx?)$/.test(path)) return "";
    const undefinedSetters = findUndefinedSetters(content, path);
    if (undefinedSetters.length === 0) return "";
    return `\n\nWARNING: The following setter(s) are called but never declared via useState (or destructured/passed as a prop) in ${path}: ${undefinedSetters.join(
      ", "
    )}. This WILL throw a ReferenceError at runtime. Fix this immediately with another str_replace before finishing your turn.`;
  }
  ...
  createFileWithParents(path: string, content: string = ""): string {
    ...
    return `File created: ${path}${this.validateContent(path, content)}`;
  }
  ...
  replaceInFile(path: string, oldStr: string, newStr: string): string {
    ...
    return `Replaced ${occurrences} occurrence(s) of the string in ${path}${this.validateContent(
      path,
      updatedContent
    )}`;
  }
  ...
  insertInFile(path: string, insertLine: number, text: string): string {
    ...
    return `Text inserted at line ${insertLine} in ${path}${this.validateContent(
      path,
      updatedContent
    )}`;
  }
